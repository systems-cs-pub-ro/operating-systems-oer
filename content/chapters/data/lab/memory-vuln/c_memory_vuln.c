...
AttackOnTitanCharacter* createElem_bad(size_t id, char *name)
{
    // allocation happens here
    AttackOnTitanCharacter p;
    //AttackOnTitanCharacter* q;
    //q = &p;
    p.id = id;
    p.name = name;
    return p;
...
void main()
{
    ...
    AttackOnTitanCharacter* oops = createElem_bad(11, "Founding Titan");

    printCharacterList(list, N);

    printf("Can read from expired stack frame id: %ld and name: %s\n");

    // no problem at all
    printf("Can read out of bounds list[20]->id: %ld\n", list[20]->id);
